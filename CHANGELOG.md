...
- **下载/解析偶发失败硬化（PR-K，5 修复）** — 用户报告"卡 90%+" + "退避似乎没生效"诊断后定位 5 个独立根因：
  ...
  2. **`fetch_range` 永久错快速失败（A）（invariant #20）** — 之前仅识别 503，4xx（403/404/410，CDN signed URL 过期典型 status）走 bytes() 路径触发"short read"被错当瞬态反复 retry 5 次。改为 `Result<Vec<u8>, HttpFailureKind>` 通过 `from_response` 按 status 分类，4xx 永久错立即 propagate 不浪费 retry budget。这是"卡 90%+"最后一 chunk 反复失败的核心根因之一。
  3. **`RetryPolicy` SOT 单源（B）（invariant #21）** — `policy.rs::from_runtime_config` / `default_for_profile` / `ranged.rs::build_policy` / `single_stream` 内联构造四套退避数学不一致（违反铁律 §3.2）。统一收敛到 `RetryPolicy::for_profile_with_max_retries(max_retries, profile)` 单源 ctor；删除 `build_policy` + `single_stream` 内联；`max_retries` 语义统一为"retry 次数"（不含首次 attempt），baseline Parse=2/Download=4 retries。`single_stream` 与 `ranged_remaining_pwrite` 不再接 `max_retries` 参数。
...
- **PR-K2 audit-all 必修小改 + max_retries 真传（4 修复，对应 PR-K merge 后 audit-all 5 FAIL + 15+ WARN 中用户认可的两组）**：
  1. **with_retry 退避表 ±50% jitter**（audit-resilience FAIL 闭合）— `executor.rs::apply_jitter` 仅作用于本地 backoff 表 fallback 路径，**不**对 `kind.retry_after()` 服务端建议加 jitter（铁律 §10：尊重服务端建议原值）。批量场景多客户端同时触发瞬态错（CDN 5xx / -460）时防 thundering herd 同步重试加深限流。
  2. **下载侧 `RetryPolicy` 真消费 `config.max_retries`（invariant #21 真兑现）** — 之前 `single_stream::download_single_stream` + `ranged::download_adaptive` (probe) + `ranged::download_remaining_and_pwrite` (chunk) 三处全走 `default_for_profile` 忽略 `DownloadConfig.max_retries`；admin UI 改 `max_retries=1` 应急止血 / `=15` 高 CDN 抖动场景缓冲均无效。本 PR 给 `download_single_stream` + `download_remaining_and_pwrite` 加 `&DownloadConfig` 参数（编译期强制调用方传配置），三处构造点统一走 `RetryPolicy::for_profile_with_max_retries(config.max_retries, Download)`。CLAUDE.md 不变量 #21 表述同步更新——SOT 自声称已交付的诚实性闭合（架构-sentinel FAIL #1 闭合）。
  3. **5 处 `matches!(...)` → `assert!(matches!(...))` 真断言**（test-gate W1-W5 闭合）— `http/error.rs` 5 处弱断言（仅返 bool 不让测试 fail）改为 `assert!`；`Permanent4xx { status: 404 }` 加 if-let 解构断 status==404 防"碰巧返 4xx 但 status 错误"假绿。
  4. **doc drift / CHANGELOG 反向引用**（audit-deprecation × 2 WARN + audit-source-of-truth A3 FAIL 闭合）— `request_with_retry` 签名 `Result<Response>` → `Result<String>` 两处 doc 同步；`RETRY_DELAYS_MS` 老 const block 改指向 `policy.rs::DEFAULT_BACKOFF` SOT 引用 + `<!-- DO NOT EDIT, see policy.rs -->` 标记；上面 PR-K 5 修复点末已加 `(invariant #19/#20/#21)` 反向引用，CLAUDE.md SOT 表与 CHANGELOG 形成闭环。
- PR-K2 配套测试 +2：`executor.rs::jitter_does_not_break_existing_retries`（200 次采样验证 wait ∈ [0.5x, 1.5x] base 区间）+ `single_stream.rs::download_single_stream_uses_config_max_retries`（验证 config.max_retries=1 → max_attempts=2，=15 clamp 到 baseline=5）。
