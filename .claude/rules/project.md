...
> Commit message 多行写法 / 裸 `@` subject 防护已升级到全局 CLI + hook 兜底——
> 见 `~/.claude/disciplines/architecture-and-format.md` "Commit message 一律走
> `dream git commit --stdin`" 节。项目级不再重复 SOT。
