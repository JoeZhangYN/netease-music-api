...
| 9 | Slider 边界单源（validate ↔ 视图 slider 同源不漂） | `kernel::runtime_config::bounds` 常量（raw 单位，每可调数值字段一条 `Bound`）——`RuntimeConfig::validate()` 与视图 slider 反退化锁 `crates/adapter/tests/admin_config_ui_coverage.rs::slider_bounds_stay_within_validate` 同读；视图 slider 经单位换算回 raw 后须 ⊆ 该 `Bound`（双侧硬界须完整覆盖合法域）。PR-10 的 `GET /admin/config/schema` 本欲做边界 SOT，但前端迁 Maud SSR 后**零消费者**且其 default/bound 已实际漂移（ranged_threads 8≠4 等），孤岛必漂活样本——v4 拆桥砍除（router 删路由 + handler 删 fn），边界单源回归 `bounds` 常量 | HTML/JS/Rust 三处边界漂移（pre-PR-10）；孤岛 schema 端点 default 自漂（无消费者无人发现）；视图硬编码边界与 validate 各持一份（pre-本次，无反退化锁）|
| 10 | Quality 列表单源 | `Quality::ALL`（PR-4 SOT）——视图 `view::admin::quality_select` + `/api/info` handler 两 live consumer 直接进程内消费。PR-10 的 `GET /admin/qualities` 端点同欲做前端 SOT，迁 Maud SSR 后亦零消费者——v4 与 schema 端点同批拆桥砍除（音质单源不变，回归 `Quality::ALL`，反退化锁 `admin_config_ui_coverage.rs::quality_fallback_floor_select_lists_all_quality_variants`）| HTML 4 select 硬编码（pre-PR-4/10，`info.rs` 漏 `dolby`）|
