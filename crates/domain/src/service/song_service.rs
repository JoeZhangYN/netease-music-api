...
pub async fn handle_name(api: &dyn MusicApi, music_id: &str) -> Result<Value, AppError> {
    // type=name 是原始网易云详情透传端点（外部 JSON 契约）；返回完整 envelope。
    Ok(api.get_song_detail(music_id).await?.into_raw())
}
...
pub async fn handle_json(
    ...
    let detail = api.get_song_detail(music_id).await?;
    ...
    let ar_name = song.artists.replace('/', ", ");
