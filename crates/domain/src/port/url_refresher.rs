//! PR-R1 — 续传 URL 刷新端口（outbound port，依赖倒置方向①）。
//!
...
//!   让引擎只依赖**自己定义的端口**，infra/handler 提供 impl。
...
pub trait UrlRefresher: Send + Sync {
    /// 为指定歌曲 + pin 的 quality 重新取一个全新的一次性 URL。
    ///
    /// `quality` 必须是 `.part` 的**实际生效** quality（非 requested），保证续传
    /// 字节连续（#14）。实现内部可叠加 ladder 降级，但必须在 `RefreshedUrl.quality`
    /// 回报实际取到的 quality，供调用方校验是否与 `.part` 一致。
    async fn refresh(
        &self,
        song_id: &str,
        quality: Quality,
        cookies: &HashMap<String, String>,
    ) -> Result<RefreshedUrl, AppError>;
}
